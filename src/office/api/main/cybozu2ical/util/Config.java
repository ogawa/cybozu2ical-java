...
public class Config {
  ...
  /**
   * アカウントファイルの officeURL キーで指定された値を返します。<br />
  ...
  public URI getOfficeURL() {
    try {
      return new URI(getValue(ConfigKeys.OFFICEURL));
    } catch (Exception e) {
      return null;
    }
  }
  ...
  public String getExportDir() {
    return getValue(ConfigKeys.EXPORTDIR);
  }

  /**
   * アカウントファイルの keyitem キーで指定された値を返します。<br />
   * キーが存在しない場合は null.
   * 
   * @return java.lang.String 登録対象ユーザーの識別項目
   */
  public String getKeyItem() {
    return getValue(ConfigKeys.KEYITEM);
  }
  ...
  public enum ConfigKeys {
    USERNAME("username"), PASSWORD("password"), OFFICEURL("officeURL"), SCHEME(
        "scheme"), CREATEDTIME("created"), EXPIREDTIME("expires"), KEYITEM(
        "keyitem"), EXPORTDIR("exportdir");
